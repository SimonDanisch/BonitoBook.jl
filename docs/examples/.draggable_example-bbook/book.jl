...
function make_draggable_cell(cell_editor, initial_x::Int, initial_y::Int)
    ...
    cell_id = "cell-$(cell_editor.uuid)"
    drag_handle_id = "drag-handle-$(cell_editor.uuid)"
    ...
    drag_js = js"""
        (function() {
            ...
            const handle = document.getElementById($drag_handle_id);
...
function Bonito.jsrender(session::Session, canvas::DraggableBook)
    ...
    return Bonito.jsrender(session, DOM.div(elements, draggable_styles, canvas_workspace))
end
