...
function visualize_mentions(mentions)
    mention_items = []
    ...
    for (entry, url) in mentions
        ...
        push!(mention_items, DOM.div(
            ...
            DOM.div(
                class="inline-block",
                style="min-width: 80px; color: var(--text-secondary); font-size: 12px; font-family: monospace; margin-top: 2px;",
            ...
            DOM.a(
                ...
                href=link,
                target="_blank",
                style="flex: 1; color: var(--text-primary); font-size: 13px; text-decoration: none; line-height: 1.4; word-wrap: break-word; cursor: pointer;",
    ...
    return DOM.div(
        ...
        DOM.div(
            ...
            DOM.h4(
                "🎬 Mentions ($(length(mention_items)) found)",
...
function find_mentions(regex)
    # Better word boundary matching for Makie aliases
    mentions = []
    ...
    return visualize_mentions(mentions)
end
```
```julia true false true
find_mentions(r"\b(plotting|plots|plot|visualization)\b")
```
```julia true false true
find_mentions(r"\b(makia|makitech|maki|makie|macki|mackie)\b")
```
```julia true false true
